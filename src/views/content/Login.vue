...
export default {
    ...
    rules: {
      empty: (v) => !!v.trim() || 'Поле не может быть пустым.',
			symbol: (v) => !/[\s\\@\\\#%\^\&\*\(\)\[\]\+\_\{\}\`\~\=\|]/.test(v) || 'Некорректные данные.'
    }
	}),
	...
	methods: {
		...
		handleSubmit() {
			if(!this.username || !this.password || !this.valid) return this.$refs.form.validate()
