         ...
         <v-simple-table border="1" width="100%">
           <thead>
             <tr>
                ...
                <th>Стоимость</th>
                <th>Статус</th>
                <th>{{ type == 1 ? "Заявки на продажу" : "Заявки на покупку" }}</th>
             </tr>
           </thead>
           <tbody>
            <tr v-for="order in getOrderByType(type)" :key="order.id" style="padding:50px 10px">
             <td>{{order.created_at}}</td>
             <td>{{order.id}}</td>
             <td colspan="2">
                <p class="text-decoration-underline font-weight-bold ma-0 pa-0" style="cursor:pointer;" @click="viewOrder(order.id)">{{order.title}}</p>
             ...
             <td class="text-center">
                <v-btn class="white--text" small color="#78C3CC">
                    Подать заявку
                </v-btn>
            </td>
           </tr>
